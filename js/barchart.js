class Barchart {
  ...
  updateVis() {
    ...
    vis.yScale.domain([0, d3.max(vis.data, d => d.num)]);
  ...
  renderVis() {
    ...
    const bars = vis.chart.selectAll('.bar')
      ...
      .attr('height', d => vis.height - vis.yScale(d.num)
      )
      .attr('y', d => vis.yScale( d.num))
      .attr('fill', d => vis.config.colorScale( d.name))
      .on('click', function (e, d) {
        ...
        if(isActive) {
          vis.dispatcher.call("filterRegion", e, "")
        } else {
          vis.dispatcher.call("filterRegion", e, d.name)
        }
